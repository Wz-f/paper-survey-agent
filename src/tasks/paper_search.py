...
def _build_arxiv_client(page_size: int = 10) -> arxiv.Client:
    """根据环境变量创建 arxiv 客户端，支持代理配置"""
    # page_size 与 max_results 保持一致，避免请求多余数据触发 429
    client = arxiv.Client(page_size=page_size, delay_seconds=5.0, num_retries=3)
    proxy = os.environ.get("HTTPS_PROXY") or os.environ.get("HTTP_PROXY")
    if proxy:
        logger.info(f"arxiv 客户端使用代理: {proxy}")
